...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Start 30s session probe
      probeTimerRef.current = setTimeout(async () => {
        try {
          const { data: { session } } = await supabase.auth.getSession();
          if (session?.user) {
            const { data: isAdminProbe } = await supabase.rpc('has_role', {
              _user_id: session.user.id,
              _role: 'admin' as any,
            });
            if (isAdminProbe) {
          ...
          toast.error("Connection is very slow. Please try again.");
          setIsLoading(false);
        } catch {
      ...
      if (roleError) {
        console.error("[AdminLogin] Role check failed:", roleError.message);
      }
